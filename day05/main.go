...
func main() {
	...
	c := make(chan int, len(seedPairs))

	for _, pair := range seedPairs {
		fmt.Println("processing: ", pair)
		go mapSeeds(pair, seed2soil, soil2fert, fert2water, water2light, light2temp, temp2humid, humid2loca, c)
	}
	// Collect results
	var allResults []int
	for i := 0; i < len(seedPairs); i++ {
		result := <-c
		fmt.Println("Received:", result)
		allResults = append(allResults, result)
	}
	fmt.Println("locapart 2 len:", len(allResults)) //10, naive approach was 2221837783
	fmt.Println("Part 2:", slices.Min(allResults))  //still 81956384
}

func mapSeeds(pair pairs, seed2soil []mapping, soil2fert []mapping, fert2water []mapping, water2light []mapping,
	light2temp []mapping, temp2humid []mapping, humid2loca []mapping, c chan int) {
